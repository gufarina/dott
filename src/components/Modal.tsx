      ...
      <div className={s.overlay} onClick={e => e.target === e.currentTarget && onClose()}>
        <div className={s.card} style={{ width }} role="dialog" aria-modal="true" aria-label={title}>
          <div className={s.header}>
            ...
            <Button variante="icon" onClick={onClose} title="Fechar" aria-label="Fechar">
...
/* Atomos do modal viraram apelido dos componentes canonicos de
   `components/ui/` (TASK-461). A receita visual e a MESMA - ela nasceu aqui e
   foi promovida pra la sem mudar declaracao; o que muda e existir um lugar so
   no app inteiro. Os nomes ModalInput/ModalButton continuam exportados porque
   ha consumidor em varias telas: apelido nao quebra ninguem. */
export const ModalInput = forwardRef<HTMLInputElement, React.InputHTMLAttributes<HTMLInputElement>>(
  function ModalInput(props, ref) {
    return <Input ref={ref} variante="campo" {...props} />
  }
)
...
export function ModalButton({
  ...
  // `hoverZoom` e `hoverGlow` sao classes GLOBAIS (reset.css), nao do modal -
  // continuam sendo aplicadas aqui, do mesmo jeito e na mesma ordem de antes.
  const extra = variant === 'primary' ? 'hoverZoom hoverGlow' : 'hoverZoom'
  return <Button variante={variant} className={`${extra} ${className ?? ''}`} {...props} />
}
