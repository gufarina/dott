...
export default function App() {
  ...
  useEffect(() => {
    ...
    let unlistenVaultReadFailed: (() => void) | undefined
    import('@tauri-apps/api/event').then(({ listen }) => {
      listen('inbox-changed', () => useStore.getState().reloadInbox())
        .then(fn => { unlistenInbox = fn })
        .catch(() => {})
      // AVISO DE NOTA ILEGIVEL: o Rust as vezes nao consegue ler um .md do
      // vault (antivirus/OneDrive/indexador segurando o arquivo aberto por um
      // instante) - a nota continua no disco, so nao entrou na lista desta
      // vez. Sem isto o usuario via menos notas que o esperado e achava que
      // tinha perdido alguma.
      listen<{ count: number; files: string[] }>('vault-read-failed', e => {
        console.error('[vault-read-failed]', e.payload)
        const { count } = e.payload
        showToast(
          'warn', 'Não consegui abrir uma nota',
          count === 1
            ? 'Uma nota não abriu agora, mas ela continua salva no seu computador.'
            : `${count} notas não abriram agora, mas elas continuam salvas no seu computador.`,
        )
      })
        .then(fn => { unlistenVaultReadFailed = fn })
        .catch(() => {})
    ...
    return () => { unlistenInbox?.(); unlistenAtalho?.(); unlistenVaultReadFailed?.() }
  }, [])
