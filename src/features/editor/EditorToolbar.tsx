...
export function EditorToolbar({ view, onPickImage, className }: Props) {
  ...
  return (
    <div className={className ? `${s.bar} ${className}` : s.bar}>
      <Button className={s.btn} title="Título" aria-label="Título"
        onClick={() => runCmd(v => v.dispatch(toggleLinePrefix(v.state, '# ', ['## ', '### '])))}>
        <Icon name="titulo" size={14} />
      </Button>
      <Button className={s.btn} title="Negrito (Ctrl+B)" aria-label="Negrito"
        onClick={() => runCmd(v => v.dispatch(toggleWrap(v.state, '**')))}>
        <Icon name="negrito" size={14} />
      </Button>
      <Button className={s.btn} title="Itálico (Ctrl+I)" aria-label="Itálico"
        onClick={() => runCmd(v => v.dispatch(toggleWrap(v.state, '_')))}>
        <Icon name="italico" size={14} />
      </Button>
      <span className={s.sep} />
      <Button className={s.btn} title="Lista" aria-label="Lista"
        onClick={() => runCmd(v => v.dispatch(toggleLinePrefix(v.state, '- ', ['* ', '+ '])))}>
        <Icon name="listaPontos" size={14} />
      </Button>
      <Button className={s.btn} title="Checklist" aria-label="Checklist"
        onClick={() => runCmd(v => v.dispatch(toggleChecklist(v.state)))}>
        <Icon name="tarefa" size={14} />
      </Button>
      <Button className={s.btn} title="Citação" aria-label="Citação"
        onClick={() => runCmd(v => v.dispatch(toggleLinePrefix(v.state, '> ', [])))}>
        <Icon name="citacao" size={14} />
      </Button>
      <span className={s.sep} />
      <Button className={s.btn} title="Código" aria-label="Código"
        onClick={() => runCmd(v => v.dispatch(toggleWrap(v.state, '`')))}>
        <Icon name="codigo" size={14} />
      </Button>
      <Button className={s.btn} title="Link" aria-label="Link"
        onClick={() => runCmd(v => v.dispatch(insertLink(v.state, false)))}>
        <Icon name="link" size={14} />
      </Button>
      <span className={s.sep} />
      <Button className={s.btn} title="Imagem" aria-label="Imagem" onClick={pickImageAtCursor}>
