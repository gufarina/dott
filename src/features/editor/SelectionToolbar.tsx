...
export function SelectionToolbar({ view, info, containerRef }: Props) {
  ...
  return (
    <div className={s.toolbar} style={{ top, left }} onMouseDown={e => e.preventDefault()}>
      <Button className={s.btn} title="Negrito (Ctrl+B)" onClick={() => run(v => v.dispatch(toggleWrap(v.state, '**')))}>
        <Icon name="negrito" size={14} />
      </Button>
      <Button className={s.btn} title="Itálico (Ctrl+I)" onClick={() => run(v => v.dispatch(toggleWrap(v.state, '_')))}>
        <Icon name="italico" size={14} />
      </Button>
      <span className={s.sep} />
      <Button className={s.btn} title="Link" onClick={() => run(v => v.dispatch(insertLink(v.state, false)))}>
