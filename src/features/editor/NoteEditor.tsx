...
export function NoteEditor() {
  ...
  /** onBlur do titulo salva NA HORA - precisa cancelar o debounce do corpo
   *  (onChange, 800ms) ANTES de gravar, senao as duas gravacoes da MESMA
   *  nota disparam quase juntas e podem se atropelar (TASK-495). Mesmo
   *  padrao de flushIfPending: mata o timer, tira o marcador PENDENTE, e
   *  so entao salva - com readLiveBody() o corpo salvo e sempre o mais
   *  recente (doc vivo do CodeMirror), corpo E titulo nunca perdem nada. */
  const saveTitle = () => {
    if (!current) return
    const titleChanged = title !== current.title
    const bodyPending = pendingSave.current
    if (!titleChanged && !bodyPending) return
    clearTimeout(saveTimer.current)
    pendingSave.current = false
    whenNoImageInFlight(() => saveNote(current.id, title, readLiveBody()))
  }
    ...
    <div className={s.wrap} ref={wrapRef}>
      ...
      <div className={s.tags}>
        ...
        {current.tags.length === 0 && !addingTag ? (
          <Button className={s.tagAdd} onClick={() => setAddingTag(true)}>
            + etiqueta
          </Button>
        ) : (
          <Input
