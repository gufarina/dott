...
import { TAG_TOKENS, resolveTagColor } from '../../lib/tagPalette'
...
export function Constellation() {
  ...
  /** SVG (fill/stroke por indice ciclico) nao entende `var()` de forma
   *  confiavel aqui - resolve o valor real dos 8 tokens de dado no tema
   *  vigente. RE-RESOLVE sempre que `theme` mudar (dep do useMemo), pra
   *  nunca ficar presa na cor do tema em que carregou. */
  const cores = useMemo(
    () => TAG_TOKENS.map((_, i) => resolveTagColor(i)),
    [theme],
  )
