...
import { tagVar } from './lib/tagPalette'
...
interface AppState {
  ...
  /** Troca so o ROTULO da pasta. Nota e tarefa continuam amarradas pelo id
   *  (folderId), entao nada muda de lugar nem se perde ao renomear. Nome
   *  vazio e ignorado - pasta sem nome fica inalcancavel na tela. */
  renameFolder: (categoryId: string, folderId: string, name: string) => void
 ...
 *  TASK-461: os 8 tons vivem em `--tag-1..8` (tokens.css); aqui so o indice
 *  vira `var(--tag-N)` via `tagPalette.ts` - nunca mais hex cravado aqui. */
...
export const useStore = create<AppState>((set, get) => ({
  ...
  inbox: INITIAL_INBOX,
  tags: [
    { name: 'tutorial', color: tagVar(0), count: 5 },
    { name: 'inicio', color: tagVar(1), count: 1 },
    { name: 'para', color: tagVar(2), count: 1 },
    { name: 'wikilinks', color: tagVar(3), count: 1 },
    { name: 'captura', color: tagVar(4), count: 1 },
    { name: 'inbox', color: tagVar(5), count: 1 },
  ],
  tasks: SEED_TASKS,
  ...
  /** Ver doc em AppState.renameFolder. */
  renameFolder: (categoryId, folderId, name) => {
    ...
    set(s => {
      ...
      const next = {
        ...
        [categoryId]: {
          ...q,
          folders: q.folders.map(f => (f.id === folderId ? { ...f, name: novo } : f)),
        },
      }
      saveFolders(next)
      return { para: next }
