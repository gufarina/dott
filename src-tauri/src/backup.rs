...
fn copy_dir(from: &Path, to: &Path) -> std::io::Result<()> {
    copy_dir_skip(from, to, None)
}

/// `skip` e o caminho canonico da pasta de backups: se uma entrada canonicaliza
/// pra ele, e pulada - evita a recursao infinita de copiar a pasta de backups
/// pra dentro de um backup (o `data_dir` pai contem a pasta de backups quando
/// nao ha `document_dir` disponivel e ela cai em `data_dir`).
fn copy_dir_skip(from: &Path, to: &Path, skip: Option<&Path>) -> std::io::Result<()> {
...
pub fn backup_now(app: tauri::AppHandle, label: String) -> Result<String, String> {
    ...
    let skip = root.canonicalize().ok();
    copy_dir_skip(&src, &dest, skip.as_deref()).map_err(|e| e.to_string())?;
